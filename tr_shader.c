...
infoParm_t	infoParms[] = {
	...
	{"water",		1,	0,	Q3CONTENTS_WATER },
	{"slime",		1,	0,	Q3CONTENTS_SLIME },		// mildly damaging
	{"lava",		1,	0,	Q3CONTENTS_LAVA },		// very damaging
	{"playerclip",	1,	0,	Q3CONTENTS_PLAYERCLIP },
	{"monsterclip",	1,	0,	Q3CONTENTS_MONSTERCLIP },
	{"nodrop",		1,	0,	Q3CONTENTS_NODROP },		// don't drop items or leave bodies (death fog, lava, etc)
	{"nonsolid",	1,	SURF_NONSOLID,	0},						// clears the solid flag

	// utility relevant attributes
	{"origin",		1,	0,	Q3CONTENTS_ORIGIN },		// center of rotating brushes
	{"trans",		0,	0,	Q3CONTENTS_TRANSLUCENT },	// don't eat contained surfaces
	{"detail",		0,	0,	Q3CONTENTS_DETAIL },		// don't include in structural bsp
	{"structural",	0,	0,	Q3CONTENTS_STRUCTURAL },	// force into structural bsp even if trnas
	{"areaportal",	1,	0,	Q3CONTENTS_AREAPORTAL },	// divides areas
	{"clusterportal", 1,0,  Q3CONTENTS_CLUSTERPORTAL },	// for bots
	{"donotenter",  1,  0,  Q3CONTENTS_DONOTENTER },		// for bots

	{"fog",			1,	0,	Q3CONTENTS_FOG},			// carves surfaces entering
...
static qboolean ParseShader( char **text )
{
	...
	//
	if ( s == 0 && !shader.isSky && !(shader.contentFlags & Q3CONTENTS_FOG ) ) {
		return qfalse;
	}
...
static shader_t *GeneratePermanentShader( void ) {
	...
	*newShader = shader;
	...
	} else if ( shader.contentFlags & Q3CONTENTS_FOG ) {
...
static shader_t *FinishShader( void ) {
    ...
    // check for a missing texture
		...
		//
		if ( pStage->isDetail && !r_detailTextures->value ) {
	...
	//
	if ( stage > 1 && ( (r_vertexLight->value && !r_uiFullScreen->value) || glConfig.hardwareType == GLHW_PERMEDIA2 ) ) {
		VertexLightingCollapse();
		stage = 1;
		hasLightmapStage = qfalse;
	}
...
shader_t *R_FindShader( const char *name, int lightmapIndex, qboolean mipRawImage ) {
	...
	strlcpy(shader.name, strippedName, sizeof(shader.name));
...
qhandle_t RE_RegisterShaderFromImage(const char *name, int lightmapIndex, image_t *image, qboolean mipRawImage) {
	...
	memset( &shader, 0, sizeof( shader ) );
	memset( &stages, 0, sizeof( stages ) );
	strlcpy(shader.name, name, sizeof(shader.name));
	shader.lightmapIndex = lightmapIndex;
	for ( i = 0 ; i < MAX_SHADER_STAGES ; i++ ) {
		stages[i].bundle[0].texMods = texMods[i];
	}
...
static void CreateInternalShaders( void ) {
	...
	strlcpy( shader.name, "<default>", sizeof( shader.name ) );
	...
	strlcpy( shader.name, "<stencil shadow>", sizeof( shader.name ) );
