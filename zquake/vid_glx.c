...
// oldman: needs this typedef to be here..
typedef void (APIENTRY *lpMTexFUNC) (GLenum, GLfloat, GLfloat);
lpMTexFUNC qglMultiTexCoord2f = NULL;

// oldman: setup gamma variables
float vid_gamma = 1.0;
byte vid_gamma_table[256];

unsigned short	d_8to16table[256];
unsigned d_8to24table[256];
unsigned d_8to24table2[256]; // oldman: added to match winGL
unsigned char	d_15to8table[65536];

// oldman: various variables required by gamma settings -->
...
static int scr_width, scr_height, scrnum; // oldman: added scrnum (gamma stuff)
...
static int XLateKey(XKeyEvent *ev)
{
	...
	switch(keysym)
	{
		...
		case XK_Num_Lock: key = KP_NUMLOCK; break;
...
// oldman: hide the mouse cursor when in window
...
// oldman: don't show mouse cursor icon
...
// oldman: started implementing gamma changing (doesn't work yet)
...
void VID_Init(unsigned char *palette)
{
	...
	// oldman: started adding fullscreen switches in
#ifdef USE_VMODE
...
// interpret command-line params
	// oldman: fullscreen cmdline check
#ifdef USE_VMODE
...
// set vid parameters
	...
	// oldman: check vmode extensions supported
#ifdef USE_VMODE
	// Get video mode list
	...
	// oldman: setups fullscreen size to fit display -->
#ifdef USE_VMODE
	if (vidmode_ext) {
	...
	// oldman: if fullscreen disable window manager decoration
#ifdef USE_VMODE
	if (vidmode_active) {
