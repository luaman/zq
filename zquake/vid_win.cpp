...
#include "common.h"
#include "cl_screen.h"		// block_drawing
#include "winquake.h"
#include "cdaudio.h"
#include "d_local.h"
#include "r_local.h"
...
// empty while Quake starts up.
	...
	vid.buffer = vid.direct = (pixel_t *)dibdc->surface;
...
EXTERNC void VID_LockBuffer (void)
{
	...
	if (memdc)
	{
		...
		vid.buffer = vid.direct = (pixel_t *)memdc->surface;
		vid.rowbytes = memdc->mi.bytesPerLine;
	}
	else if (mgldc)
	{
		// Update surface pointer for linear access modes
		vid.buffer = vid.direct = (pixel_t *)mgldc->surface;
...
EXTERNC extern void M_Print (int cx, int cy, char *str);
EXTERNC extern void M_PrintWhite (int cx, int cy, char *str);
