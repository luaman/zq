...
static void AssertCvarRange( cvar_t *cv, float minVal, float maxVal, qboolean shouldBeIntegral )
{
	if ( shouldBeIntegral )
	{
		if ( ( int ) cv->value != cv->value )
		{
			ri.Printf( PRINT_WARNING, "WARNING: cvar '%s' must be integral (%f)\n", cv->name, cv->value );
			ri.Cvar_Set( cv->name, va( "%d", cv->value ) );
		}
	}
...
void GfxInfo_f( void ) 
{
	...
	ri.Printf( PRINT_ALL, "MODE: %d, %d x %d %s hz:", r_mode->value, glConfig.vidWidth, glConfig.vidHeight, fsstrings[r_fullscreen->value == 1] );
	...
	if ( r_vertexLight->value || glConfig.hardwareType == GLHW_PERMEDIA2 )
	{
...
//	Swap_Init();
	...
	if ( (intptr_t)tess.xyz & 15 ) {
		ri.Printf( PRINT_WARNING, "WARNING: tess.xyz not 16 byte aligned\n" );
	}
	...
	max_polys = r_maxpolys->value;
	if (max_polys < MAX_POLYS)
		max_polys = MAX_POLYS;

	max_polyverts = r_maxpolyverts->value;
