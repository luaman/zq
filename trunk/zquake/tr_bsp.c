...
static	void R_ColorShiftLightingBytes( byte in[4], byte out[4] ) {
	...
	shift = r_mapOverBrightBits->value - tr.overbrightBits;
...
static void ParseFace( dsurface_t *ds, drawVert_t *verts, msurface_t *surf, int *indexes  ) {
	...
	if ( r_singleShader->value && !surf->shader->isSky ) {
...
qboolean R_GetEntityToken( char *buffer, int size ) {
	...
	s = COM_ParseQ3( &s_worldData.entityParsePoint );
...
void RE_LoadWorldMap( const char *name ) {
	...
	strlcpy( s_worldData.name, name, sizeof( s_worldData.name ) );

	strlcpy( s_worldData.baseName, COM_SkipPath( s_worldData.name ), sizeof( s_worldData.name ) );
