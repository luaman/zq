...
start:
	...
	// move up to the node
	nearside = (t1 < t2) ? 1 : 0;
	int check = RecursiveHullTrace (htl, node->children[nearside], p1f, midf, p1, mid);
	if (check == TR_BLOCKED)
		return check;
...
trace_t CM_HullTrace (hull_t *hull, vec3_t start, vec3_t end)
{
	...
	int check = RecursiveHullTrace (&htl, hull->firstclipnode, 0, 1, start, end);
