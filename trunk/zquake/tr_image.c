						  ...
						  int *pUploadWidth, int *pUploadHeight )
{
	...
	if (!lightMap) {
		...
		if ( samples == 3 )
		{
			...
			else if ( r_texturebits->value == 16 )
			{
				internalFormat = GL_RGB5;
			}
			else if ( r_texturebits->value == 32 )
			{
	...
	if (mipmap)
	{
		if ( textureFilterAnisotropic )
			qglTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT,
					(GLint)Com_Clamp( 1, maxAnisotropy, r_ext_max_anisotropy->value ) );
...
void R_LoadImage( const char *name, byte **pic, int *width, int *height )
{
	...
	*height = 0;
	...
	strlcpy( localName, name, MAX_QPATH );
...
void R_SetColorMappings( void ) {
	...
	tr.overbrightBits = r_overBrightBits->value;
	if ( !glConfig.deviceSupportsGamma ) {
...
qhandle_t RE_RegisterSkin( const char *name ) {
	...
	while ( text_p && *text_p ) {
		...
		strlcpy( surfName, token, sizeof( surfName ) );
		...
		strlcpy( surf->name, surfName, sizeof( surf->name ) );
...
void	R_InitSkins( void ) {
	...
	strlcpy( skin->name, "<default skin>", sizeof( skin->name )  );
	skin->numSurfaces = 1;
